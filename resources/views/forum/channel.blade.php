    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                ...
                <ul class="space-y-2">
                    <li>
                        <a href="{{ route('forum.index') }}" class="block px-3 py-2 rounded text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
                            All Discussions
                        </a>
                    </li>
                    @foreach($channels as $ch)
                        <li>
                            <a href="{{ route('channel.show', $ch) }}" class="block px-3 py-2 rounded {{ $ch->id === $channel->id ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700' }}">
                                {{ $ch->title }}
                                <span class="text-xs text-gray-500 dark:text-gray-400">({{ $ch->discussions_count }})</span>
                            </a>
                        </li>
                    @endforeach
                </ul>
        ...
        <!-- Main Content - Channel Discussions -->
        <div class="lg:col-span-3">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
                <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $channel->title }}</h1>
                    @if($channel->description)
                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">{{ $channel->description }}</p>
                    @endif
                </div>
                ...
                @else
                    <div class="px-6 py-12 text-center">
                        ...
                        <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">No discussions in this channel</h3>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Be the first to start a discussion in {{ $channel->title }}.</p>
                        @auth
                            <div class="mt-6">
