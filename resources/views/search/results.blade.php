...
@section('description', "Found {$discussions->total()} results for '{$query}' in our developer forum.")
        ...
        <div class="lg:col-span-3">
            ...
            <div class="mb-6">
                ...
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                    Found {{ $discussions->total() }} {{ Str::plural('result', $discussions->total()) }} for "<span class="font-semibold">{{ $query }}</span>"
                </p>
            </div>
            ...
            <div class="mb-6">
                <form action="{{ route('search') }}" method="GET" class="relative">
                    ...
                    <input
                        ...
                        class="w-full pl-10 pr-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                        autofocus
                    >
                </form>
            </div>
            ...
            @if($discussions->count() > 0)
                            ...
                            <div class="flex gap-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-400 line-clamp-2">
                                        {{ Str::limit(strip_tags(markdown($discussion->content)), 150) }}
                                    </p>
                ...
                @if($discussions->hasPages())
                    <div class="mt-6 flex justify-center">
                        {{ $discussions->appends(['q' => $query])->links() }}
                    </div>
                @endif
            @else
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-12 text-center">
                    ...
                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                        We couldn't find any discussions matching "<span class="font-semibold">{{ $query }}</span>".
                        <br>Try different keywords or browse all discussions.
                    </p>
                    <div class="mt-6">
                        <a href="{{ route('forum.index') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-lg font-medium text-sm text-white hover:bg-indigo-700 transition">
