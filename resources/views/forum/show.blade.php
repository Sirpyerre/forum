    ...
    @auth
        <div class="mt-8 bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <form action="{{ route('replies.store', $discussion) }}" method="POST">
                @csrf
                <div>
                    <textarea name="content" rows="4" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="Write your reply..." required>{{ old('content') }}</textarea>
                    @error('content')
